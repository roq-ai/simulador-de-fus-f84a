...
import { getGameLevelById, updateGameLevelById } from 'apiSdk/game-levels';
...
function GameLevelEditPage() {
  ...
  const handleSubmit = async (values: GameLevelInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateGameLevelById(id, values);
      mutate(updated);
      resetForm();
