...
import { getGameProgressById } from 'apiSdk/game-progresses';
import { GameProgressInterface } from 'interfaces/game-progress';

function GameProgressViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<GameProgressInterface>(
    () => (id ? `/game-progresses/${id}` : null),
    () =>
      getGameProgressById(id, {
        relations: ['user'],
      }),
  );
        ...
        {isLoading ? (
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem label="Race Won" text={data?.race_won} />

                <FormListItem label="Car Transformation" text={data?.car_transformation} />
