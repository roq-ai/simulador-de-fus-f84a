...
import { getGameProgressById, updateGameProgressById } from 'apiSdk/game-progresses';
...
function GameProgressEditPage() {
  ...
  const handleSubmit = async (values: GameProgressInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateGameProgressById(id, values);
      mutate(updated);
      resetForm();
