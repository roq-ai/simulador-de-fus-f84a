...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getGameLevels() {
    ...
    const response = await prisma.game_level
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'game_level'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createGameLevel() {
    await gameLevelValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.game_level.create({
      data: body,
    });
